...
func (c *Cache) WriteExpiryFile() error {
	...
	err = os.WriteFile(filename, []byte(sj), 0640)
	if err != nil {
		return err
	}
...
func (c *Cache) getURLFromAPI(itemKey string) (gj gjson.Result, err error) {
	...
	log.Infof("Requested retrieval of: %s", itemKey)
	...
	// We have successfully retrieved a URL so update its cache expiry time.
...
func (c *Cache) jsonToFile(filename string, gj gjson.Result) (err error) {
	...
	err = os.WriteFile(filename, jBytes, 0640)
	return
}
